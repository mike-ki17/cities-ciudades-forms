...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Gráfica de respuestas mensuales -->
        <div class="admin-card mb-8">
            <div class="px-6 py-4 border-b" style="border-color: var(--color-border);">
                <h3 class="text-lg leading-6 font-medium admin-text">Respuestas Mensuales</h3>
                <p class="text-sm admin-text-secondary">Evolución de respuestas en los últimos 12 meses</p>
            </div>
            <div class="p-6">
                <div class="chart-container">
                    <canvas id="submissionsByMonthChart"></canvas>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Gráfica de respuestas mensuales
    try {
        const submissionsByMonthCtx = document.getElementById('submissionsByMonthChart').getContext('2d');
        ...
        for (let i = 11; i >= 0; i--) {
            ...
            monthlyLabels.push(`${monthNames[date.getMonth()]} ${year}`);
            
            // Buscar datos para este mes
            const monthData = statistics.submissions_by_month.find(m => m.year === year && m.month === month);
            monthlyData.push(monthData ? monthData.count : 0);
        }
        
        console.log('Creating monthly chart with data:', { labels: monthlyLabels, data: monthlyData });
        
        new Chart(submissionsByMonthCtx, {
            type: 'line',
            data: {
