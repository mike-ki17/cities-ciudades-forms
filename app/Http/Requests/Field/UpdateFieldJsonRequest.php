...
class UpdateFieldJsonRequest extends FormRequest
{
    /**
    ...
    private function validateFieldStructure($validator): void
    {
        ...
        if (!isset($field['key']) || empty($field['key'])) {
            ...
            // Check if key is unique (excluding current field)
            $existingField = \App\Models\FieldJson::where('key', $field['key'])->where('id', '!=', $fieldId)->first();
            if ($existingField) {
        ...
        if (!isset($field['type']) || empty($field['type'])) {
            ...
            $validTypes = ['text', 'email', 'number', 'textarea', 'select', 'checkbox', 'date', 'section', 'tel', 'file'];
